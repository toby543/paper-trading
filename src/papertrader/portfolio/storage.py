...
class Storage:
    ...
    def _init_db(self, starting_capital: float) -> None:
        with self._conn() as conn:
            ...
            if "rationale" not in existing_cols:
                conn.execute("ALTER TABLE trades ADD COLUMN rationale TEXT")
    ...
    def record_trade(self, trade: Trade) -> int:
        """Insert `trade` and return its new row id -- callers that want to
        attach something after the fact (e.g. an LLM rationale, generated
        only once the trade is already safely recorded) need it to target
        the right row."""
        with self._conn() as conn:
            cur = conn.execute(
                """INSERT INTO trades (symbol, side, quantity, price, charges, reason, timestamp, realized_pnl, rationale)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (trade.symbol, trade.side, trade.quantity, trade.price, trade.charges, trade.reason,
                 trade.timestamp, trade.realized_pnl, trade.rationale),
            )
            return cur.lastrowid

    def set_trade_rationale(self, trade_id: int, rationale: str) -> None:
        """Attach a rationale to an already-recorded trade. Separate from
        record_trade because the rationale (if any) is generated by a
        network call made strictly after the trade is placed -- the trade
        must never wait on it, and must never be lost if it fails."""
        with self._conn() as conn:
            conn.execute("UPDATE trades SET rationale = ? WHERE id = ?", (rationale, trade_id))
    ...
    def get_trades(self, limit: int | None = None) -> list[Trade]:
        ...
        return [
            Trade(
                ...
                rationale=r["rationale"] if "rationale" in r.keys() else None,
            )
            for r in rows
        ]
